...
export class UDashboardComponent implements OnInit, AfterViewInit {
  ...
  async fetchDocumentStats(): Promise<void> {
    try {
      ...
      this.documentStats = [
        { title: 'Total Documents', value: incomingCount + receivedCount + outgoingCount, icon: 'fas fa-file-alt', iconColor: 'text-orange-500' }, // file icon
        { title: 'Total Incoming', value: incomingCount, icon: 'fas fa-inbox', iconColor: 'text-orange-500' }, // inbox icon
        { title: 'Total Received', value: receivedCount, icon: 'fas fa-folder-open', iconColor: 'text-orange-500' }, // folder-open icon
        { title: 'Total Outgoing', value: outgoingCount, icon: 'fas fa-paper-plane', iconColor: 'text-orange-500' }, // paper-plane icon
        { title: 'Total Completed', value: outgoingCount, icon: 'fas fa-check-circle', iconColor: 'text-orange-500' } // check-circle icon
      ];
